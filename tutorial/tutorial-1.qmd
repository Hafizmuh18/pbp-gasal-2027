...
::: {.callout-note}
Perhatikan ada **dua** folder bernama `myportofolio` - folder terluar adalah root proyekmu (tempat `manage.py` berada), sedangkan folder di dalamnya adalah *package* konfigurasi Django (tempat `settings.py` dan `urls.py` berada). Ini bukan duplikasi yang salah, memang begitu struktur bawaan `startproject`.
:::

## Menyiapkan Views, URL, Templates, dan Static Files

Buat sebuah *view* sederhana yang mengembalikan sebuah *template* HTML. Buat berkas `myportofolio/views.py`:
...
Hubungkan *view* ini ke URL kosong (`""`) di `myportofolio/urls.py`:
...
Buat folder `templates/`, `static/css/`, dan `static/img/` di root proyek (sejajar dengan `manage.py`), lalu daftarkan `templates/` dan `static/` di `myportofolio/settings.py` supaya Django tahu harus mencari berkas HTML dan aset statis di sana:
...
7. Pada `settings.py` di proyek Django kamu, tambahkan URL *deployment* PWS ke `ALLOWED_HOSTS`.
   ...
   ::: {.callout-warning}
   Sebelum lanjut ke langkah berikutnya, pastikan struktur repositorimu lengkap: berkas `manage.py`, `requirements.txt`, `.gitignore`, direktori `myportofolio/` (folder konfigurasi proyek), dan **bukan** direktori `env/` atau berkas `db.sqlite3` (harusnya sudah di-*gitignore*).
...
- **Berkas `.gitignore`**: pastikan diawali titik, tidak punya ekstensi tambahan, dan berada di *root folder* bersama `manage.py`, direktori `myportofolio/`, `requirements.txt`, `.git`, dan `db.sqlite3`.
...
::: {.callout-note}
Notice there are **two** folders named `myportofolio` - the outer one is your project root (where `manage.py` lives), while the one inside it is the Django configuration *package* (where `settings.py` and `urls.py` live). This isn't a mistake, it's `startproject`'s default structure.
:::

## Set Up Views, URLs, Templates, and Static Files

Create a simple *view* that returns an HTML *template*. Create `myportofolio/views.py`:
...
Connect this view to the empty (`""`) URL in `myportofolio/urls.py`:
...
Create a `templates/`, `static/css/`, and `static/img/` folder at the project root (next to `manage.py`), then register `templates/` and `static/` in `myportofolio/settings.py` so Django knows to look there for HTML pages and static assets:
...
7. In your Django project's `settings.py`, add the PWS *deployment* URL to `ALLOWED_HOSTS`.
   ...
   ::: {.callout-warning}
   Before moving to the next step, make sure your repository structure is complete: `manage.py`, `requirements.txt`, `.gitignore`, the `myportofolio/` config directory, and **not** an `env/` directory or a `db.sqlite3` file (those should already be *gitignored*).
...
- **`.gitignore` file**: make sure it starts with a dot, has no extra extension, and sits at the *root folder* alongside `manage.py`, the `myportofolio/` directory, `requirements.txt`, `.git`, and `db.sqlite3`.
