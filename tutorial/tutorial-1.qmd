...
10. Kalau nanti ada perubahan pada proyek Django yang mau kamu push ke PWS lagi, kamu cukup jalankan (setelah `add` dan `commit`):
    ...
    ::: {.callout-note}
    Jika *branch* lokalmu bernama `master` (bukan `main`), gunakan `git push pws master:main` supaya tetap terkirim ke *branch* `main` di remote PWS.
    :::
...
- Kalau struktur sudah yakin lengkap tapi tetap gagal, pastikan kamu push ke PWS mengarah ke *branch* `main` (`git push pws main`).
...
Kalau kamu membuat proyek baru di PWS untuk mengatasi masalah proyek sebelumnya, ganti *remote* URL di repositori lokalmu supaya `git push pws main` mengarah ke proyek yang baru, bukan yang lama:
...
10. If you make changes to your Django project later that you want to push to PWS, just run this (after `add` and `commit`):
    ...
    ::: {.callout-note}
    If your local branch is named `master` (not `main`), use `git push pws master:main` so it still lands on the `main` branch on the PWS remote.
    :::
...
- If you're sure the structure is complete but it still fails, make sure you're pushing to PWS targeting the `main` branch (`git push pws main`).
...
If you created a new PWS project to work around an issue with a previous one, update the *remote* URL in your local repository so `git push pws main` points to the new project instead of the old one:
