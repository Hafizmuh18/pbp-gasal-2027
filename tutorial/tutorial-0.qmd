...
## Menyiapkan Repository myportfolio
...
2. Buka [GitHub](https://github.com), klik tombol **+** di pojok kanan atas lalu pilih **New repository**. Isi `Repository name` dengan `myportfolio`, pilih visibilitas **Public**, biarkan opsi lain (README, .gitignore, license) tetap kosong/default, lalu klik **Create repository**.
...
![Repositori myportfolio di GitHub setelah push pertama](../img/tutorial-0/github-repo-created.png)
...
1. Buka halaman repositori `myportfolio` yang sudah kamu buat di [GitHub](https://github.com).
...
Setelah ini kamu punya tiga repositori: repositori asli di komputer lokal, repositori daring di GitHub, dan repositori baru hasil *cloning* yang terhubung ke repositori GitHub yang sama. Ini memungkinkanmu bekerja dengan repositori yang sama dari beberapa tempat/komputer dengan mudah.
...
1. Buat dan pindah ke *branch* baru dengan perintah `git checkout -b <NAMA_BRANCH>`.
   ...
   ::: {.callout-tip}
   Kalau kamu ingin berpindah ke *branch* yang **sudah ada** (bukan membuat baru), jalankan `git checkout <NAMA_BRANCH>` saja tanpa flag `-b`. Flag `-b` khusus dipakai untuk membuat *branch* baru sekaligus langsung berpindah ke sana.
   :::

2. Ubah sedikit isi `README.md`, lalu simpan, `add`, `commit`, dan push *branch* tersebut.
...
3. Buka repositorimu di GitHub. Secara otomatis akan muncul *pop-up* dengan tombol **Compare & pull request**. Kalau tidak muncul, alternatifnya tekan tombol **Pull Request**, lalu pilih **New pull request**. GitHub akan membandingkan perubahan di kedua *branch* yang ingin digabungkan.
...
Di dalam folder `myportfolio`, buat *virtual environment*.
...
**Dependencies** adalah *library*/*package* yang dibutuhkan aplikasimu untuk berjalan. Buat berkas `requirements.txt` berisi daftar *dependencies* berikut (tanpa perlu menentukan versi spesifik):
...
Lalu buat proyek Django bernama `myportfolio`:
...
::: {.callout-important}
Pastikan karakter `.` tertulis di akhir perintah - ini membuat Django meletakkan `manage.py` langsung di folder `myportfolio`, bukan di dalam subfolder baru.
:::
...
Buat berkas `.gitignore` di root proyek (sejajar dengan `manage.py`) supaya `env/`, berkas `.env`/`.env.prod`, `db.sqlite3`, dan berkas sementara lain tidak ikut ter-*commit*:
...
::: {.callout-important}
Berkas `.env` dan `.env.prod` **tidak boleh** ikut ter-*push* ke GitHub karena (nantinya) berisi kredensial - pastikan `.gitignore` di atas sudah dibuat SEBELUM kamu menjalankan `git add`.
:::
...
::: {.callout-important}
**Menghubungkan ke Proyek Kamu** - repositori `myportfolio` yang baru kamu buat ini **adalah** website portofolio pribadimu untuk sepanjang semester, bukan latihan sekali pakai. Struktur, riwayat commit, dan branch yang kamu buat di sini akan terus berkembang di tutorial-tutorial berikutnya.
:::
...
## Setting Up the myportfolio Repository
...
2. Open [GitHub](https://github.com), click the **+** button in the top right corner and choose **New repository**. Fill in `Repository name` with `myportfolio`, choose **Public** visibility, leave the other options (README, .gitignore, license) empty/default, then click **Create repository**.
...
![The myportfolio repository on GitHub after the first push](../img/tutorial-0/github-repo-created.png)
...
1. Open your `myportfolio` repository page on [GitHub](https://github.com).
...
You now have three repositories: the original one on your local machine, the remote one on GitHub, and the new cloned copy connected to that same GitHub repository. This lets you work with the same repository from multiple places easily.
...
1. Create and switch to a new branch with `git checkout -b <BRANCH_NAME>`.
   ...
   ::: {.callout-tip}
   If you want to switch to a branch that **already exists** (not create a new one), just run `git checkout <BRANCH_NAME>` without the `-b` flag. The `-b` flag is specifically for creating a new branch and switching to it in one step.
   :::

2. Make a small change to `README.md`, save it, then `add`, `commit`, and push that branch.
...
3. Open your repository on GitHub. A **Compare & pull request** *pop-up* will appear automatically. If it doesn't, click the **Pull Request** button instead, then choose **New pull request**. GitHub will compare the changes between the two branches you want to merge.
4. If there's no conflict, click **Merge pull request** to merge the changes from `latihan-branch` into `main`.
...
Inside the `myportfolio` folder, create a *virtual environment*.
...
**Dependencies** are the libraries/packages your app needs to run. Create a `requirements.txt` file with the following dependencies (no need to pin specific versions):
...
Then create the Django project named `myportfolio`:
...
::: {.callout-important}
Make sure the `.` character is at the end of the command - this makes Django place `manage.py` directly in the `myportfolio` folder instead of inside a new subfolder.
:::
...
1. Create a `.env` file at the project root (next to `manage.py`) with:
...
3. Add the following near the top of `settings.py` (after the `Path` import), then add `ALLOWED_HOSTS` and `PRODUCTION`:
...
4. Find the `DATABASES` section in `settings.py` and replace it with configuration that automatically picks SQLite (development) or PostgreSQL (production):
...
Create a `.gitignore` file at the project root (next to `manage.py`) so `env/`, the `.env`/`.env.prod` files, `db.sqlite3`, and other temporary files don't get committed:
...
::: {.callout-important}
The `.env` and `.env.prod` files must **not** be pushed to GitHub since they (eventually) hold credentials - make sure the `.gitignore` above exists BEFORE you run `git add`.
:::
...
::: {.callout-important}
**Connecting to Your Project** - the `myportfolio` repository you just created **is** your personal portfolio website for the whole semester, not a one-off exercise. The structure, commit history, and branches you build here will keep growing in the following tutorials.
:::
