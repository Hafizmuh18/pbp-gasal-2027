...
Rentang versi tersebut membatasi `pip` (*package manager* Python) agar memasang Django versi 5.2 ke atas dan di bawah versi 6 (`5.2 <= x < 6.0`).
Hal ini bertujuan untuk memastikan versi Django yang dipakai selama perkuliahan PBP tetap konsisten di versi 5.2 LTS,
...
This version range tells `pip` (Python's package manager) to install the latest Django release from version 5.2 up to, but excluding, version 6.0 (`5.2 <= x < 6.0`).
Version pinning ensures the Django version used throughout PBP coursework stays consistent at version 5.2 LTS,
