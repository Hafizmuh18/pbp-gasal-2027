...
:::
  ...
  6. In the **index.html** file located in the **templates** directory, move only the `<main>` element and its entire contents into the content block. You do not need to rewrite `<header>` and `<footer>`, as they are already included in `base.html`. For example:
...
Before we start the implementation, form tightly coupled with CSRF token, with that we must register our PWS website URL. Open `settings.py`, then add this line:
```py 
# gunakan https:// untuk trailing urlnya
CSRF_TRUSTED_ORIGINS = ["<url_website_kamu>"]
```
...
- `CSRF Token` is a secret unique token created by server in purpose to secure application from malicious and unauthorized request.
- `CSRF_TRUSTED_ORIGINS` register trusted URL to enable request process into server.

The Main Purpose for CSRF implementation beside obligation from Django Form creation, is to prevent attacker to change request source from our Django Server into some malicious API.
...
- `widgets` controls the appearance and the HTML element used by each field. In the code above, `TextInput` is used for single-line text fields, `Textarea` is used for a description that needs a larger text area, and `URLInput` is used for URL fields. Attributes inside `attrs`, such as  `placeholder`, `maxlength`, and `rows`, used for  hint text, character limits, and the height of the text area.
