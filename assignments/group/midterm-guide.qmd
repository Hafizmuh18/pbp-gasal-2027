...
Untuk keperluan database yang digunakan dalam tugas kelompok, kamu dapat menggunakan schema `tugas_kelompok` dari salah satu database anggota kelompok kamu yang sebelumnya sudah diberikan oleh tim ITF (lihat variabel `SCHEMA` di [Tutorial 0](../../tutorial/tutorial-0.qmd) untuk konteks konfigurasinya).
...
For database purposes used in group projects, you can use `tugas_kelompok` schema from one of your group member's database that was previously provided by ITF team (see the `SCHEMA` variable in [Tutorial 0](../../tutorial/tutorial-0.qmd) for context on its configuration).
