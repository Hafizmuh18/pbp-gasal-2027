  ...
  build-and-push:
    ...
    - name: Push to PWS
      ...
      run: |
          ...
          # Push current branch to PWS main branch
          push_output=$(git push $PWS_URL $current_branch:main 2>&1)
          if [[ $? -ne 0 ]]; then
...
::: {.callout-note}
Versi *action* (`actions/checkout@v7`) di atas adalah versi terkini saat panduan ini ditulis. Cek [GitHub Marketplace](https://github.com/marketplace/actions/checkout) sebelum memakainya untuk memastikan kamu masih memakai versi terbaru - *action* versi lama (misalnya `@v2` ke bawah) memakai *runtime* Node.js yang sudah *deprecated* dan bisa gagal berjalan di *runner* GitHub Actions saat ini.
:::
...
## 1. Microsoft Planner
![Gambar MS Planner](../../img/group/ms-planner.png)
...
## 2. Linear.app
![Gambar Dashboard Linear App](../../img/group/linear.jpg)
...
## 3. Notion
![Gambar Notion](../../img/group/notion.png)
...
## 4. ClickUp
![Gambar Click Up](../../img/group/clickup.png)
...
::: {.callout-note}
The *action* version above (`actions/checkout@v7`) is the current version as of when this guide was written. Check the [GitHub Marketplace](https://github.com/marketplace/actions/checkout) before using it to make sure you're still on the latest version - older *action* versions (e.g. `@v2` and below) use a Node.js *runtime* that's been *deprecated* and can fail to run on current GitHub Actions *runners*.
:::
...
## 1. Microsoft Planner
![MS Planner Image](../../img/group/ms-planner.png)
...
## 2. Linear.app
![Linear App Dashboard Image](../../img/group/linear.jpg)
...
## 3. Notion
![Notion Image](../../img/group/notion.png)
...
## 4. ClickUp
![ClickUp Image](../../img/group/clickup.png)
